...
void SHA512::run(){
    ...
    data += unhexlify(makehex(length & mod64, 32));
    for(unsigned int n = 0; n < (data.size() >> 7); n++){
        ...
        for(uint8_t x = 16; x < 80; x++){
            skey[x] = (s1(skey[x - 2]) + skey[x - 7] + s0(skey[x - 15]) + skey[x - 16]) & mod64;
        }
        uint64_t a = h0, b = h1, c = h2, d = h3, e = h4, f = h5, g = h6, h = h7;
        for(uint8_t x = 0; x < 80; x++){
            uint64_t t1 = (h + S1(e) + Ch(e, f, g) + SHA512_K[x] + skey[x]) & mod64;
            uint64_t t2 = (S0(a) + Maj(a, b, c)) & mod64;
            h = g;
            g = f;
            f = e;
            e = (d + t1) & mod64;
