...
std::string PGP_compress(const uint8_t alg, const std::string & src){
    if ((alg != Compression::Algorithm::UNCOMPRESSED) && src.size()){ // if the algorithm value is not zero and there is data
        bool good = false;
        std::string dst;

        switch (alg){
            case Compression::Algorithm::ZIP:
                good = (zlib_compress(src, dst, DEFLATE_WINDOWBITS, Z_DEFAULT_COMPRESSION) == Z_OK);
                break;
            case Compression::Algorithm::ZLIB:
                good = (zlib_compress(src, dst, ZLIB_WINDOWBITS, Z_DEFAULT_COMPRESSION) == Z_OK);
                break;
            case Compression::Algorithm::BZIP2:
